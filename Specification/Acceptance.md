...
- The four-service Docker Compose stack starts SQL Server, RabbitMQ Management, API, and WorkerService with named volumes, a named network, dependency health checks, generated ignored credentials, and loopback-only host ports.
...
- The Next.js and Angular applications remain workspace siblings as requested. Their inclusion in one Week 4 CI checkout is blocked on the repository-boundary decision recorded in `docs/adr/005-workspace-repository-boundary.md`.
